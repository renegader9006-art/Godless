...
def analyze_html(source: str, html: str, keywords: list[str] | None = None) -> AnalysisResult:
    ...
    has_breadcrumb_schema = any(
        "breadcrumblist" in (s.get_text(" ", strip=True) or "").lower() for s in ld_scripts
    )
    ...
    _audit_tech(issues, title, description, has_lang_attr, has_viewport, has_noindex, canonical, og_title, og_description, schema_count)
    _audit_structure(issues, len(h1_tags), len(h2_tags), heading_jumps, duplicate_block_pct, has_loading_placeholder, script_to_text_ratio)
    _audit_content(issues, word_count, paragraph_count, avg_sentence_len, long_paragraph_share, unique_ratio, risky_copy_hits)
    _audit_intent(issues, intent_coverage_pct, faq_signal_count, cta_hits, keywords, title, h1_tags, text_lower)
    _audit_media_links(issues, alt_coverage, internal_links, external_links, generic_anchor_share)
    _audit_trust(issues, trust_signal_score, has_author_signal, has_date_signal, has_contact_signal, has_breadcrumb_schema)
...
def analyze_text_article(
    ...
    if not title.strip():
        _add_issue(issues, "MEDIUM", CAT_STRUCTURE, "Отсутствует заголовок статьи", "Добавьте конкретный заголовок под интент пользователя.", 5)
    elif len(title.strip()) < 30:
        _add_issue(issues, "LOW", CAT_STRUCTURE, "Заголовок слишком короткий", "Уточните заголовок и добавьте конкретику.", 2)
...
def _audit_tech(
    ...
    if len(title) < 35:
        _add_issue(issues, "HIGH", CAT_TECH, "Недостаточно информативный title", "Перепишите title в диапазоне 45-65 символов.", 9)
    elif len(title) > 70:
        _add_issue(issues, "MEDIUM", CAT_TECH, "Слишком длинный title", "Сократите title, чтобы он не обрезался в выдаче.", 4)
    if len(description) < 90:
        _add_issue(issues, "MEDIUM", CAT_TECH, "Meta description отсутствует или слабый", "Добавьте meta description 110-160 символов.", 4)
    ...
    if not has_lang_attr:
        _add_issue(issues, "LOW", CAT_TECH, "Не указан lang", "Добавьте атрибут lang в html.", 2)
    if not has_viewport:
        _add_issue(issues, "MEDIUM", CAT_TECH, "Не указан viewport", "Добавьте meta viewport для мобильной адаптации.", 4)
    if has_noindex:
        _add_issue(issues, "HIGH", CAT_TECH, "Обнаружен noindex", "Проверьте meta robots: noindex блокирует индексацию.", 10)
    if not canonical:
        _add_issue(issues, "LOW", CAT_TECH, "Отсутствует canonical", "Добавьте canonical.", 2)
    if not og_title or not og_description:
        _add_issue(issues, "LOW", CAT_TECH, "Неполные OpenGraph-метаданные", "Добавьте og:title и og:description.", 2)
    if schema_count == 0:
        _add_issue(issues, "LOW", CAT_TECH, "Нет JSON-LD разметки", "Добавьте структурированные данные, где это уместно.", 2)
...
def _audit_structure(
    ...
    if h1_count != 1:
        _add_issue(issues, "HIGH", CAT_STRUCTURE, "Некорректный H1", "На странице должен быть один основной H1.", 9)
    if h2_count < 2:
        _add_issue(issues, "MEDIUM", CAT_STRUCTURE, "Слабая иерархия подзаголовков", "Добавьте H2/H3 для структурирования контента.", 4)
    if heading_jumps > 0:
        _add_issue(issues, "LOW", CAT_STRUCTURE, "Нарушена иерархия заголовков", "Исправьте прыжки уровней заголовков.", 2)
    if duplicate_block_pct >= 35:
        _add_issue(issues, "MEDIUM", CAT_STRUCTURE, "Повторяющиеся текстовые блоки", "Снизьте дублирование контента в DOM.", 4)
    if has_loading_placeholder:
        _add_issue(issues, "MEDIUM", CAT_STRUCTURE, "Признаки незавершенного рендера", "Проверьте, что виден финальный контент, а не placeholder.", 4)
    if script_to_text_ratio > 0.15:
        _add_issue(issues, "LOW", CAT_STRUCTURE, "Высокая доля скриптов", "Проверьте риск потери контента при клиентском рендере.", 2)
...
def _audit_content(
    ...
    if word_count < 140:
        _add_issue(issues, "HIGH", CAT_CONTENT, "Недостаточная глубина контента", "Добавьте больше фактической пользы, шагов и примеров.", 9)
    elif word_count < 260:
        _add_issue(issues, "MEDIUM", CAT_CONTENT, "Контент можно усилить", "Добавьте конкретику и практические детали.", 4)
    if paragraph_count < 3:
        _add_issue(issues, "MEDIUM", CAT_CONTENT, "Слабая структура абзацев", "Разбейте текст на короткие смысловые абзацы.", 4)
    if avg_sentence_len > 24:
        _add_issue(issues, "MEDIUM", CAT_CONTENT, "Слишком длинные предложения", "Сократите среднюю длину предложений.", 4)
    if long_paragraph_share > 45:
        _add_issue(issues, "LOW", CAT_CONTENT, "Много длинных абзацев", "Разбейте длинные блоки на более короткие.", 2)
    if unique_ratio < 0.35 and word_count > 150:
        _add_issue(issues, "LOW", CAT_CONTENT, "Низкое лексическое разнообразие", "Снизьте повторяемость формулировок.", 2)
    if risky_copy_hits >= 4:
        _add_issue(issues, "MEDIUM", CAT_CONTENT, "Рискованный copywriting", "Снизьте агрессивные обещания и добавьте проверяемые аргументы.", 4)
    elif risky_copy_hits >= 2:
        _add_issue(issues, "LOW", CAT_CONTENT, "Есть признаки агрессивного copy", "Смягчите рекламные формулировки.", 2)
...
def _audit_intent(
    ...
    if intent_coverage_pct < 45:
        _add_issue(issues, "HIGH", CAT_INTENT, "Слабое покрытие интента", "Добавьте практические блоки: как сделать, примеры, ошибки.", 9)
    elif intent_coverage_pct < 70:
        _add_issue(issues, "MEDIUM", CAT_INTENT, "Полезность можно усилить", "Углубите ответ под запрос пользователя.", 4)
    if faq_signal_count == 0:
        _add_issue(issues, "LOW", CAT_INTENT, "Нет FAQ-сигнала", "Добавьте блок частых вопросов/ответов.", 2)
    if cta_hits == 0:
        _add_issue(issues, "LOW", CAT_INTENT, "Нет явного следующего шага", "Добавьте понятный CTA в конце контента.", 2)
    ...
    if keywords:
        ...
        if not any(k.lower() in core for k in keywords):
            _add_issue(issues, "MEDIUM", CAT_INTENT, "Ключи не отражены в title/H1", "Добавьте ключевой интент в title или H1.", 4)
        if not any(k.lower() in text_lower for k in keywords):
            _add_issue(issues, "LOW", CAT_INTENT, "Ключи не раскрыты в тексте", "Убедитесь, что целевые темы реально раскрыты в контенте.", 2)
...
def _audit_media_links(
    ...
    if alt_coverage < 85:
        _add_issue(issues, "MEDIUM", CAT_MEDIA, "Низкое покрытие alt у изображений", "Добавьте осмысленные alt-тексты.", 4)
    if internal_links < 2:
        _add_issue(issues, "LOW", CAT_MEDIA, "Мало внутренних ссылок", "Добавьте ссылки на связанные страницы.", 2)
    if external_links == 0:
        _add_issue(issues, "LOW", CAT_MEDIA, "Нет внешних подтверждающих источников", "Добавьте релевантные внешние источники, где это уместно.", 2)
    if generic_anchor_share >= 60:
        _add_issue(issues, "LOW", CAT_MEDIA, "Слабые анкоры ссылок", "Замените общие анкоры на более информативные.", 2)
...
def _audit_trust(
    ...
    if trust_signal_score <= 1:
        _add_issue(issues, "MEDIUM", CAT_TRUST, "Слабые сигналы доверия", "Добавьте автора, дату и контактные данные.", 4)
    if not has_author_signal:
        _add_issue(issues, "LOW", CAT_TRUST, "Не указан автор", "Добавьте автора/эксперта материала.", 2)
    if not has_date_signal:
        _add_issue(issues, "LOW", CAT_TRUST, "Не указана дата", "Добавьте дату публикации или обновления.", 2)
    if not has_contact_signal:
        _add_issue(issues, "LOW", CAT_TRUST, "Нет контактного сигнала", "Добавьте контакты или ссылку на страницу контактов.", 2)
    if not has_breadcrumb_schema:
        _add_issue(issues, "LOW", CAT_TRUST, "Нет breadcrumb-разметки", "Добавьте breadcrumb-разметку для структуры.", 1)
...
def _estimate_intent_coverage(
    text_lower: str, h2_tags: list, keywords: list[str], title: str, h1_tags: list
) -> float:
    checks = 0
    score = 0
    ...
    checks += 1
    if any(p in text_lower for p in ("пример", "кейс", "ошибка", "вопрос")):
        score += 1

    checks += 1
    if any(p in text_lower for p in ("шаг", "чек-лист", "что делать", "вывод")):
        score += 1
...
def _faq_signal_count(text: str, has_faq_schema: bool) -> int:
    ...
    if has_faq_schema:
        count += 1
    if "faq" in lower or "частые вопросы" in lower:
        count += 1
    if text.count("?") >= 2:
        count += 1
    return count
...
def _add_keyword_density_hints(
    issues: list[SEOIssue], keyword_stats: dict[str, float], keywords: list[str] | None
) -> None:
    ...
    for keyword, density in keyword_stats.items():
        if density > 4.5:
            _add_issue(
                ...
                "LOW",
                CAT_INTENT,
                f"Возможен переспам ключом: {keyword}",
                "Плотность ключевых слов используйте как вторичный сигнал: снижайте повторы и добавляйте синонимы.",
                penalty=1,
            )
        elif 0 < density < 0.15:
            _add_issue(
                ...
                "LOW",
                CAT_INTENT,
                f"Ключ почти не встречается: {keyword}",
                "Если ключевой запрос действительно важен, естественно добавьте его в релевантные разделы.",
                penalty=1,
            )
